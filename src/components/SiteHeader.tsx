    ...
    <header className="sticky top-0 z-40 border-b border-brand-rule bg-brand-paper/90 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {NAV.map((item) => {
            // Find food — prominent, underlined text link (no pill).
            if (item.href === "/resources") {
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className="rounded-full px-3 py-1.5 text-[15px] font-semibold text-brand-ink underline decoration-2 underline-offset-4 transition hover:bg-brand-cream"
                >
                  {item.label}
                </Link>
              );
            }
            // Login — the primary CTA pill.
            if (item.href === "/account") {
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className="ml-1 rounded-full bg-brand-navy px-5 py-2 text-sm font-semibold text-white hover:bg-brand-navy/90"
                >
                  {item.label}
                </Link>
              );
              ...
              if (item.href === "/resources") {
                return (
                  <Link
                    ...
                    className="rounded-lg px-3 py-2.5 text-[15px] font-semibold text-brand-ink underline decoration-2 underline-offset-4 hover:bg-brand-cream"
                  >
                    {item.label}
                  </Link>
                );
              ...
              if (item.href === "/account") {
                return (
                  <Link
                    ...
                    href={item.href}
                    onClick={() => setOpen(false)}
                    className="mt-1 rounded-lg bg-brand-navy px-3 py-2.5 text-sm font-semibold text-white"
                  >
                    {item.label}
                  </Link>
                );
